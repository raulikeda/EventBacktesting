...
from src.event_backtesting.order import Order, Trade


def test_order_initialization_and_repr():
    timestamp = datetime.now()
    order = Order(instrument="instrument", side=Order.BUY, quantity=1, price=10)
    ...
    new_order = order = Order(
        instrument="instrument2", side=Order.SELL, quantity=2, price=20
    )
    assert new_order.id == 2


def test_order_creation():
    order = Order("AAPL", Order.BUY, 100, 150.00)
